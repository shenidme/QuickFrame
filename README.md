...
3. **松开后**默认立即开始 0.5 秒渐隐。初始预设为彩虹辉光，默认不加遮罩。
4. 右键点击托盘图标 → **设置与预览**，调整外观、触发方式和开机自启动；平时仅驻留托盘。

**Ctrl + Alt + F8** 或双击托盘图标：暂停 / 恢复。托盘图标可能位于任务栏的隐藏图标区域。

**Esc** 清除正在绘制和仍在显示的框；没有框时不会拦截 Esc。升级保留原来的设置，可在设置面板中“恢复默认”后保存。

## 功能

- 7 种风格：赛博霓虹、彩虹辉光、经典彩虹、冰蓝极光、琥珀全息、薄荷流光、夜城协议；另有 5 种纯色辉光。
- 夜城协议包含展开锁定、分段装甲、边缘光点和故障消散效果。所有图形由代码绘制。
- 聚焦遮罩让框内保持正常、框外平滑变暗；多个框分别渐隐，支持所有显示器或最近画框所在显示器。
- 动画预览和粗细、辉光强度、颜色循环、渐隐时长、遮罩深度设置。
- 可选 Alt+右键、后退侧键、前进侧键触发；支持按进程名排除应用。
- 鼠标监听运行在独立线程，提供重新连接和手动修复入口。
- 设置保存到程序旁的 `settings.ini`，下次启动恢复；绘制画布复用，空闲时释放。
- 可选开机自启动：当前用户登录 Windows 时启动，默认关闭，不需要管理员权限。启用后请保持 EXE 路径不变。
- 无安装器、无网络请求或遥测。
...
- Windows 10/11，.NET Framework 4.8 或更新版本。解压后请保留 EXE 旁的 QuickFrame.exe.config，以启用高 DPI 支持。
- 开启时，选定的鼠标拖动用于画框。需要应用原有手势或游戏操作时，可选 Alt+右键、排除应用或暂停。
- 选定鼠标键的普通单击在松开时回放，因此部分软件的按住行为可能不同。排除列表约每 250ms 检查当前前台应用。
...
交互测试会短暂显示示例框、遮罩和设置面板，验证监听线程、长按状态机和面板控件，不注入鼠标点击。冒烟测试使用独立的测试实例，不会读取或修改日常配置。注册表测试只使用临时测试键，不会启用实际自启动。
...
```text
src/QuickFrame.cs       输入监听、预设绘制和程序入口
src/Overlay.cs          分层窗口、遮罩和复用画布
src/Controller.cs       托盘和应用生命周期
src/Settings.cs         配置与当前用户自启动
src/SettingsWindow.cs   设置面板和动画预览
src/Checks.cs           回归检查
build.ps1              构建、测试和便携包
docs/                  使用说明与效果预览
